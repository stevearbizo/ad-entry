...
function concatDependencies() {
	window.adParams.dependencies = []
		...
		.concat(window.assets.payload.text)
}
...
function loadJs() {
	return new Promise((resolve, reject) => {
		var jsLoader = new Loader(window.adParams.dependencies, {
			name: 'jsLoader',
			scope: global,
			onFail: reject,
			onComplete: resolve
...
function loadFba() {
	return new Promise((resolve, reject) => {
		if (window.assets.payload.binary.length) {
			var fbaLoader = new Loader(window.adParams.adPath + window.assets.payload.binary[0], {
				name: 'fbaLoader',
				scope: global,
				fileType: 'fba',
				onFail: reject,
